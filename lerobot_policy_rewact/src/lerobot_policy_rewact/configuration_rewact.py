...
class SAM3Config:
    ...
    weights: str | None = None
    # Which SAM3 feature-pyramid level to use as tokens.
    # SAM3 neck scale_factors are [4, 2, 1, 0.5] (in that order); the "native" scale=1.0 map is index 2.
    fpn_level: int = 2
    # Positional embedding source for the returned tokens:
    # - "sam3": use SAM3's own `vision_pos_enc` for the selected feature map
    # - "act_sinusoidal": use ACT's 2D sinusoidal pos embedding on the selected feature map grid
    pos_embed_type: str = "sam3"
    # Optional 2×2 patch merge stages to reduce token count.
    # 0 = none, 1 = 4× fewer tokens, 2 = 16× fewer tokens.
    patch_merge_stages: int = 2
    # Backward-compat sugar (prefer patch_merge_stages).
    use_patch_merge: bool = False
    # SAM3 native resolution (square) used by its preprocessing.
    # RewACT will aspect-ratio resize the input and letterbox-pad to this square size.
    input_resolution: int = 1008
    # Optional torch.compile for the SAM3 backbone (can help perf, but may reduce compatibility).
